...
const electronicsRoot = resolve(process.cwd(), 'modules/electronics/editor');
...
const editorHostSource = readFileSync(
  resolve(process.cwd(), 'packages/editor-host/src/ProjectEditorHeader.tsx'),
  'utf8',
);
const editorSource = readFileSync(resolve(electronicsRoot, 'SchematicEditor.tsx'), 'utf8');
...
describe('owner-reference Electronics presentation contract', () => {
  ...
  it('matches the compact editor chrome and shape-following selection contract', () => {
    ...
    expect(editorHostSource).toContain('onViewChange(tab.id)');
