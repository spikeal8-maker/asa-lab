...
function verifyQuality(
  ...
  for (const component of document.components) {
    ...
    if (component.kind === 'potentiometer') {
      ...
      const currentA =
        (voltageA - voltageWiper) /
        Math.max(CLOSED_RESISTANCE, component.value * position);
      const currentB =
        (voltageWiper - voltageB) /
        Math.max(CLOSED_RESISTANCE, component.value * (1 - position));
...
export function analyseCircuit(document: ElectronicsDocument): SimulationResult {
  ...
  if (compiled.unsupportedComponentIds.length > 0) {
    const diagnostic: Diagnostic = {
      ...
      suggestedAction: 'Удалите неподдерживаемый компонент или дождитесь его подтверждённой модели.',
    };
    return {
      solved: false,
      status: 'unsupported',
