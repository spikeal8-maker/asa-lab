...
export type GameScopeKind = 'private' | 'classroom' | 'workspace' | 'global' | 'event' | 'tournament';
export type GameRuntimeKind = 'command' | 'realtime_room';
export type GameTopologyKind = 'duel' | 'free_for_all' | 'teams' | 'coop';
export type GameMatchStatus = 'waiting' | 'ready' | 'active' | 'finishing' | 'finished' | 'cancelled' | 'aborted';
...
export function validateGameMatchStructure(input: GameMatchStructureV1): readonly GameMatchStructureIssue[] {
  const { match, participants, teams } = input;
  const issues: GameMatchStructureIssue[] = [];

  if (!Number.isInteger(match.stateSchemaVersion) || match.stateSchemaVersion < 1 || !Number.isInteger(match.lifecycleVersion) || match.lifecycleVersion < 1) {
    issues.push({ code: 'INVALID_VERSION', message: 'stateSchemaVersion and lifecycleVersion must be positive integers.' });
  }

  if ((match.competitionKind === 'rated' && !match.ratingPolicyVersion) || (match.competitionKind === 'casual' && match.ratingPolicyVersion !== null)) {
    issues.push({ code: 'INVALID_RATING_POLICY', message: 'Rated matches require ratingPolicyVersion; casual matches require null.' });
  }

  const seats = new Set<string>();
  for (const participant of participants) {
    if (participant.matchId !== match.id) {
      issues.push({ code: 'PARTICIPANT_MATCH_MISMATCH', message: `Participant ${participant.id} belongs to another match.` });
    }
    if (seats.has(participant.seatKey)) {
      issues.push({ code: 'DUPLICATE_SEAT_KEY', message: `Duplicate seatKey ${participant.seatKey}.` });
    }
    seats.add(participant.seatKey);
    if (!participantIdentityIsValid(participant)) {
      issues.push({ code: 'INVALID_PARTICIPANT_IDENTITY', message: `Participant ${participant.id} has an invalid identity shape.` });
    }
  }
  ...
  for (const team of teams) {
    if (team.matchId !== match.id) {
      issues.push({ code: 'TEAM_MATCH_MISMATCH', message: `Team ${team.id} belongs to another match.` });
    }
  ...
  if (match.topology === 'duel') {
    if (participants.length !== 2) {
      issues.push({ code: 'INVALID_DUEL_ROSTER', message: 'Duel matches require exactly two participants in V1.' });
    }
    if (teams.length !== 0 || participants.some((participant) => participant.teamId !== null)) {
      issues.push({ code: 'UNEXPECTED_TEAMS', message: 'Duel matches must contain zero teams and null participant teamId values.' });
    }
  }

  if (match.topology === 'free_for_all') {
    if (participants.length < 2) {
      issues.push({ code: 'INVALID_FFA_ROSTER', message: 'Free-for-all matches require at least two participants.' });
    }
    if (teams.length !== 0 || participants.some((participant) => participant.teamId !== null)) {
      issues.push({ code: 'UNEXPECTED_TEAMS', message: 'Free-for-all matches must contain zero teams and null participant teamId values.' });
    }
  }

  if (match.topology === 'teams') {
    if (teams.length < 2) {
      issues.push({ code: 'INVALID_TEAM_COUNT', message: 'Team matches require at least two match-scoped teams.' });
    }
    for (const participant of participants) {
      if (!participant.teamId || !teamIds.has(participant.teamId)) {
        issues.push({ code: 'INVALID_TEAM_MEMBERSHIP', message: `Participant ${participant.id} must reference a team from the same match.` });
      }
    }
  }
  ...
  if (match.topology === 'coop') {
    if (teams.length !== 1) {
      issues.push({ code: 'INVALID_TEAM_COUNT', message: 'Co-op matches require exactly one match-scoped team.' });
    }
    for (const participant of participants) {
      if (!participant.teamId || !teamIds.has(participant.teamId)) {
        issues.push({ code: 'INVALID_TEAM_MEMBERSHIP', message: `Participant ${participant.id} must reference the cooperative team.` });
      }
    }
  }
