...
and Scratch Editor version. The Docker build checks both the fetched commit and
the root package version before it runs `npm ci`.
...
- version: `15.1.1`
- upstream license at this commit: `AGPL-3.0-only`

Do not replace the commit with `develop`, `latest` or an unpinned npm range.
Upstream updates must arrive as reviewed ASA Lab changes with compatibility
checks.
...
product should remain branded as ASA Lab visual programming rather than imply an
