...
async function openEditor(locale: string): Promise<{
  ...
  await expect(frame.locator('[data-asa-host-shell]')).toHaveAttribute('data-editor-state', 'ready', {
    timeout: 45000,
  });
...
test('English browser keeps native Scratch controls, ASA chrome and unfiltered extension catalogue', async () => {
  ...
  try {
    ...
    await page.screenshot({ path: `${evidenceDir}/01-asa-chrome-and-account.png`, fullPage: true });
...
test('ru-RU browser starts in native Russian Scratch and changes language only through Settings', async () => {
  ...
  try {
    ...
    await page.screenshot({ path: `${evidenceDir}/02-native-russian-settings.png`, fullPage: true });
    expect(external).toEqual([]);
    expect(fixture.pageErrors).toEqual([]);
  } finally {
    await closeFixture(fixture);
