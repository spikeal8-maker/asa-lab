...
Короткий контекст направления формирует `pnpm agent:context --scope <lane>`. Для
картированной локальной правки используй более узкий `--path`, `--surface` или `--control`;
команда не создаёт второго состояния и читает execution lane из `current.yaml`.
...
api:  4611
e2e: 4612
scratch runtime: 4613 (service scratch in the same Compose project)
...
проверяет происхождение контейнеров и точный SHA Web/API/Scratch.
...
Явно разрешённое владельцем регулярное обслуживание может выполняться без
повторного запроса в сохранённом окне и только для канонической установки.
Используется [переносимый менеджер](docs/deployment/PORTABLE_OPERATIONS.md),
проверяющий выпуск, полный backup, идентичность и блокировку параллельных операций.
Это разрешение не распространяется на восстановление живой БД, перенос volumes
или подключение облачного хранилища.

### 4.1. Scratch — сервис ASA Lab, не второе развёртывание

`compose.yaml` уже содержит `scratch` вместе с `web`, `api`, `migration` и
`postgres`. Собственные Docker-образ и внутренний порт — детали развёртывания
компонента, не разрешение создать другой Compose project, checkout, аккаунт или БД.
Browser origin редактора совпадает с порталом.
...
Если установка уже существует, сохраняй её каталог, project name, volume,
портовую пару и origin. При занятом порте выясни владельца и STOP: запрещено
искать свободный порт, менять loopback-адрес или запускать второй Scratch как
...
Одноразовые тесты выполняются в изолированном CI. Локальный отдельный стенд
...
## 9. Пользователи, доступы и кабинеты

Короткие неизменяемые инварианты этой области находятся в
`docs/agent/contracts/identity.yaml`; полный целевой контракт —
`docs/product/ASA_USERS_ACCESS_AND_SETTINGS_SPEC.md` и читается только как escalation по
точным разделам. Локальная правка начинает с `agent:context --path/--surface/--control`, а не
с полного Users/Access master.

Нельзя вводить глобальный `user.role`, делать организацию/школу родителем личного Account,
требовать Account для StudentSeat или считать UI/клиентский scope источником authorization.
