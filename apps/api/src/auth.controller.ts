...
/**
 * What the browser is told about a session.
 *
 * There is no role: what an account may do comes from the capabilities the
 * server granted it, and the client never states one.
 */
...
export class AuthController {
  ...
  private async payload(context: ActiveContext): Promise<SessionPayload> {
    ...
    return {
      authenticated: true,
      user: { id: context.accountId, displayName: context.displayName, email: context.email },
   ...
   *
   * It succeeds completely — account, profile, principal, creator capability,
   * Personal Workspace, session and audit event — or it changes nothing. The
   * person is signed in by the same call, so there is never an account that
   * can be created but not entered.
   */
  @Post('register')
  @HttpCode(201)
  async register(
  ...
  /**
   * The one sign-in: an email address or a username, and a password.
   *
   * A body carrying an organization code takes the legacy path, which exists
   * only for the schools onboarded before accounts did.
   */
  @Post('login')
  @HttpCode(200)
  async login(
   ...
   *
   * A visitor who presents no cookie is not an error: the honest answer is
   * "nobody yet", and the browser console stays clean on the public pages. A
   * cookie that no longer resolves is different — a credential was presented
   * and it is no good — so that still answers 401.
   */
  @Get('me')
