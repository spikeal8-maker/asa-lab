...
import { solveCircuit, parseElectronicsDocument } from '@asa-lab/electronics';
...
export class ProjectsController {
  ...
  /** The schematic result is computed server-side so the domain stays the single
   * source of truth for current, LED state and diagnostics. */
  ...
  async list(
    ...
    const result = await this.listUseCase.execute(context.tenantId, classroomId, context.userId);
    if (!result.ok) {
      ProjectsController.reject(result.code, result.message);
    }
  ...
  async create(
    ...
    const shape = checkBodyShape(rawBody, ['classroomId', 'module', 'title']);
    if (!shape.ok) {
