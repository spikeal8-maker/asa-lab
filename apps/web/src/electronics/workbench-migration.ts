...
const LEGACY_RENDER_WIDTH: Record<Exclude<ComponentKind, 'wire'>, number> = {
  source: 118,
  resistor: 164,
  led: 92,
};
 ...
 * terminal references, wire IDs/colours/vertices and electrical topology remain
 * unchanged. Immutable ProjectVersions are rendered/migrated on read later and
...
export function migrateElectronicsGeometry(
  ...
  const components = document.components.map((component) => {
    ...
    const physicalSize = Math.abs(rotation % 180) === 90
