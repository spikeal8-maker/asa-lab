...
export function MyProjectsPage({ onOpenProject }: { onOpenProject: (projectId: string) => void }): JSX.Element {
    ...
    <main className="portal-content" id="main-content">
      <section className="portal-hero">
        <div>
          ...
          <h1>Мои проекты</h1>
          <p>Создавайте демонстрации, экспериментируйте и готовьте будущие задания независимо от классов.</p>
        </div>
      ...
      {error ? <div className="portal-empty" role="alert"><p>{error}</p><button className="btn-secondary" onClick={() => void load()}>Повторить</button></div> : null}
      {items === null && !error ? <div className="project-gallery loading" aria-label="Загрузка проектов"><div/><div/><div/></div> : null}
      {items?.length === 0 ? (
        ...
        <ul className="project-gallery" data-testid="personal-project-grid">
          {items.map((project) => (
            <li key={project.id} className="project-gallery-card">
              <button type="button" className="project-preview" onClick={() => onOpenProject(project.id)}>
                <span className="project-preview-grid" aria-hidden="true"><CircuitIcon /></span>
              </button>
              <div className="project-card-meta">
                <div><h2>{project.title}</h2><p>Электроника · личный проект</p></div>
