...
Для готовых образов, полного переноса и еженедельных обновлений используйте
[переносимый менеджер](PORTABLE_OPERATIONS.md). Ему дополнительно нужен Python
3.11+. Ниже сохранён путь **сборки из исходников**: разработка или установка до
появления первого опубликованного выпуска. Эти два пути не следует смешивать
в одной управляемой установке.
...
Локальный вход: `http://127.0.0.1:4610`; редактор открывается внутри проекта
на том же сайте. Для публичного сервера используется один HTTPS-домен и один
FRP-вход на Web; отдельный домен или пользовательский порт Scratch не создаётся.
`ASA_BLOCKS_RUNTIME_ORIGIN` равен origin портала. Подробности и миграция старого
