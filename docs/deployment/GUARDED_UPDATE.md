...
Для установки с `.asa/installed-release.json` и еженедельного обслуживания
используйте [менеджер готовых выпусков](PORTABLE_OPERATIONS.md). Описанные ниже
`docker-update` предназначены для прежних установок со сборкой из исходников;
они не читают закреплённые digest и отказываются менять управляемую установку.
