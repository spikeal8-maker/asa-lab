...
| S4 | Оба gate зелёные и их результат всё ещё описывает голову | статус `PASS`, а между `verified_sha` и головой изменено только `docs/execution/**` |
...
там-то». S4 — актуальность: «этот результат всё ещё описывает голову». Смешение
этих двух вопросов было конструктивным дефектом первой версии control plane:
проверка требовала завершённого прогона на текущей голове, а первый прогон новой
головы сам задаёт этот вопрос — и блокировал бы каждый push после merge.

S4 формулируется не как равенство SHA. Запись результата — сама по себе коммит,
поэтому голова всегда как минимум на один служебный коммит впереди ревизии, на
которой gates реально прогонялись. Требование равенства делало бы подтверждённую
голову недостижимой, а не просто труднодостижимой. Результат переживает коммиты,
которые не могут изменить его исход, — то есть затрагивающие только
`docs/execution/**`. Любое изменение вне этих путей результат обесценивает.
Само служебное изменение при этом не остаётся непроверенным: второй прогон
протокола перехода прогоняет governance-gate уже поверх него.
...
2. current.yaml: каждый gate
     status: NOT_RUN
     verified_sha: null
   Не формальность: прежний результат описывает предыдущую ревизию

3. commit ЛОКАЛЬНО, без push

4. тело PR привести к SHA этого коммита
   git rev-parse HEAD даёт его до публикации

5. push
   → первый CI на новой голове
   → валидатор проходит: NOT_RUN не требует прогона,
     а тело PR уже называет голову

6. дождаться завершения обоих workflow

7. current.yaml: записать фактические результаты
     status: PASS | FAIL
     verified_sha: <голова>

8. commit локально → обновить тело PR → push
   → второй CI подтверждает, что запись соответствует наблюдению
```

### Почему коммит и push разделены

Порядок шагов 3–5 установлен исполнением, а не рассуждением, и стоил двух
падений подряд.
