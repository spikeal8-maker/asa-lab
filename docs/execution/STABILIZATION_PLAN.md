...
| S3 | Результат gate читается у того, кто его наблюдал | входит в S2 |
| S4 | Все прогоны на текущей голове зелёные | перечислить запуски на HEAD, каждый `success` |
| S5 | Открытых блокеров нет, либо каждый имеет решение владельца | `blocking` в `current.yaml` |
| S6 | PR продуктовой задачи является единицей ревью | `changed_files < 100`, рукописный код виден без фильтров |
| S7 | Каждая команда gate существует и определена ровно один раз | `validate_test_catalog.py` |

S3 и S4 когда-то были разными вопросами, потому что результат хранился в
`current.yaml` парой `status` и `verified_sha`: «что наблюдалось» и «на какой
ревизии». Копия немедленно устаревала при каждом merge, поэтому и требовалась
отдельная проверка её актуальности.

Копии больше нет. Какая ревизия зелёная — факт, который GitHub Actions держит
per-commit, и валидатор читает его оттуда. Хранить рядом вторую версию того же
факта означало заводить источник, который расходится с первым в течение часов;
именно это и происходило. Поэтому S3 сводится к «читай у наблюдателя», а S4 — к
проверяемому вопросу без косвенности: перечислить все запуски на текущей голове
и убедиться, что ни один не красный.

Перечислить **все** — существенно. Один и тот же коммит порождает прогоны от
`push` и от `pull_request`, а продуктовая ветка вдобавок запускает focused
workflow. Проверка одного завершившегося запуска однажды дала «зелёную» голову,
на которой второй запуск был красным.
...
Не ради записи результата — её больше нет. Ради того, чтобы описание перехода
никогда не называло ревизию, которой ещё нет или уже нет. Тело PR читается
workflow, запускаемым событием `pull_request`, и этот workflow стартует
одновременно с push.

Валидатор **не** требует завершённого прогона на текущей голове: первый прогон
новой головы сам задаёт этот вопрос, и такое требование блокировало бы каждый
push после merge. Привязка к голове проверяется там, где на неё опираются: при
переводе задачи в `in_review` и при оценке S4.
...
2. commit ЛОКАЛЬНО, без push

3. тело PR привести к SHA этого коммита
   git rev-parse HEAD даёт его до публикации

4. push
   → CI на новой голове; тело PR уже называет её

5. дождаться завершения ВСЕХ запусков на этой голове
   от push, от pull_request, focused workflow — каждого
```

Прежняя версия протокола насчитывала восемь шагов: два лишних обнуляли записанный
результат перед push и записывали фактический после, и требовали второго прогона
CI, чтобы подтвердить саму запись. Записи больше нет, поэтому нет и второго
прогона. Один переход — один прогон, и он же производит факт, который читает
валидатор.

### Почему коммит и push разделены

Порядок шагов 2–4 установлен исполнением, а не рассуждением, и стоил двух
падений подряд.

Сначала тело PR обновлялось после CI. Валидатор остановил переход: тело
описывало ревизию, которой больше нет, а подтверждённых результатов ещё не было.
Правильное поведение — описание обязано следовать за головой.

Затем тело обновлялось сразу после push. Это дало гонку: workflow, запускаемый
событием `pull_request`, стартует одновременно с push и успевает прочитать
прежнее тело. Прогон от `push` при этом проходил, а прогон от `pull_request` на
той же голове падал — один и тот же SHA с двумя разными вердиктами.

Гонка устраняется полностью, потому что SHA известен сразу после локального
коммита. Тело обновляется в промежутке между `git commit` и `git push`, и к
моменту, когда GitHub запускает любой workflow, описание уже верно.

### Что произойдёт, если пропустить шаг 5

Ничего немедленного, и в этом опасность. Голова окажется неподтверждённой, а
валидатор не упадёт: он не требует завершённого прогона на текущей голове, иначе
блокировал бы каждый push. Упадёт он при попытке объявить задачу готовой к ревью.

Хуже — дождаться одного запуска и счесть голову зелёной. Один и тот же коммит
порождает прогоны от `push` и от `pull_request`, а продуктовая ветка вдобавок
запускает focused workflow. Ровно так однажды и вышло: `pull_request` зелёный,
`push` красный, коммит один. Условие S4 поэтому сформулировано как «все запуски
на голове», а не «прогон прошёл».
...
после-merge голове: валидатор не требует завершённого прогона на текущей голове и
её не блокирует. Если он всё-таки упадёт — это дефект, который надо чинить, а не
обходить.
...
и по завершении перехода: все запуски на новой голове завершились `success`.
...
Численная правильность остаётся в `modules/electronics/testing/solver.spec.ts`.
...
- голова объявлена зелёной по одному запуску, когда на ней их несколько;
- результат gate снова начинает храниться в репозитории рядом с тем, что его
  наблюдает: две версии одного факта расходятся в течение часов;
