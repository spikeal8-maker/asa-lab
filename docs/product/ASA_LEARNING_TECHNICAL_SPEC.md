...
Текущий E1-контракт вставки практики ограничен canonical project activity. В **E1** UI MUST allow:

```text
выбрать существующую canonical published project LearningActivityVersion
создать новый project-материал в общей библиотеке и опубликовать его
вставить exact published Electronics/3D project activity version
```

Question Bank, Quiz Builder и создание quiz activity относятся к **E2** и не показываются в E1 как работающие действия. После начала E2 тот же insertion control может получить `Создать/выбрать quiz`, не создавая второй Course block engine. Новый материал сначала сохраняется в общей библиотеке, затем course draft ссылается на exact published version.
