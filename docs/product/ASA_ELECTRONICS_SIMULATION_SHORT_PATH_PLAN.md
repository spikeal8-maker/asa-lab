...
`deviceHealth → presentationState`. Минимально пользователь должен увидеть:

- `warning`: заметное, но не перекрывающее схему предупреждение на компоненте;
...
layers, CSS/SVG filters или заранее предусмотренные управляемые группы. Вместе
с визуалом inspector показывает входные величины, порог, состояние и время до
перехода. Цвет не является единственным каналом — нужен значок/форма/подпись.
...
13. Удаляет заменённый старый путь и повторяет gates.
14. Публикует только законченный рубеж и проверяет production exact SHA.
15. Отчитывается по контракту `AGENTS.md`.
16. Не начинает следующий этап до приёмки текущего результата.
