...
#### 4.5.1. Ученический центр обучения и путь к заданию

Нормативная детализация: [ASA_LEARNER_LEARNING_HUB_SPEC.md](learning/ASA_LEARNER_LEARNING_HUB_SPEC.md).

Обычный Account не переключается в отдельный «режим ученика». Личные проекты/лаборатории остаются личными; учебные группы, курсы и задания агрегируются в «Моём обучении».

Account может одновременно участвовать в нескольких Classroom/учебных группах. StudentSeat остаётся scoped к одному Classroom. «Моё обучение» не требует переключать Account workspace ради выбора другой учебной группы.

Основной learner journey:

~~~text
Главная / учебное внимание
→ Моё обучение
→ группа / курс / конкретное задание
→ прочитать exact task
→ Начать / Продолжить
→ предметная лаборатория
...
Обязательны два разных индикатора:
- bell badge = число непрочитанных learning notifications;
- badge «Моё обучение» = число уникальных работ, где от ученика сейчас требуется действие. Scheduled/locked, submitted-waiting-review и completed туда не входят.

Если опубликованное/выданное задание имеет future effective opensAt, ученик видит title, group/course context, opensAt и dueAt, но не может открыть полный task content или editor до открытия. Авторский draft, который ещё не выдан learner audience, невидим.

dueAt и closesAt различаются: просроченная работа может оставаться доступной по late policy; после effective closesAt Start/Submit запрещены, если teacher unlock/override не даёт иного canonical разрешения.

Клик по заданию открывает learner task detail до editor. Просмотр detail сам по себе не создаёт Project. Первый Start выполняется через canonical start contract; Resume использует существующую учебную работу.

Normal editor entry после task detail начинается с постоянной collapsed assignment anchor, а не с большой открытой панели. Anchor остаётся toggle; panel открывается над/рядом, переносится и изменяет размер на desktop. Целевые notification destinations ведут в соответствующий learning/task context, а не заставляют ученика искать Project вручную.
