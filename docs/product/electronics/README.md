...
Оставшаяся часть `MATH-4B2` включает адаптивное отклонение шага, NPN astable
fixture, накопление тепла, переход `warning → overheat → failure` и повторный
расчёт после отказа. До её приёмки статический `destructive_preview` не выдаётся
за постоянное повреждение.
