...
- `dc-motor` — обычный коллекторный двигатель постоянного тока; существующая
  модель `generic-dc-motor-static` является только временной DC-подменой и не
  закрывает этап;
- `gearmotor` — тот же класс двигателя с механическим редуктором; до реализации
  этого этапа он остаётся честным `unsupported_fail_closed`.
...
активирует `gearmotor`: его transmission profile уже проходит чистое ядро, а
production identity и полная browser-матрица остаются границами MATH-5E/F.
...
- мотор-редуктор остаётся fail-closed до MATH-5E/F.
...
MATH-5D реализует этот information/presentation contract без преждевременной
production-активации. Подтверждённый профиль `adafruit-3777-tt-48to1` связан с
fixture
...
появляются только вместе с принятым результатом solver. На Stage будущий
основной readout использует `outputRpm` и вообще не рисуется, пока каталог не
объявил модель поддержанной. Это не позволяет подготовленному UI выдать
fail-closed компонент за работающий.
...
- Stage-контракт выбирает signed `outputRpm`, но fail-closed до активации;
...
- справка простыми словами объясняет, что редуктор меняет скорость и момент, но
  не создаёт мощность;
- production identity, browser journey и реальная схема управления не
  активируются этим подпунктом и остаются в MATH-5E/F.
...
#### MATH-5F — граница активации

Срез активируется в production только когда одновременно выполнены:
