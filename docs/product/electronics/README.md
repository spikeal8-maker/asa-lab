...
индикатором; дополнительная карточка на Stage запрещена. PNP и MOSFET не
считаются частью MATH-3 и остаются на совместимом legacy-пути до MATH-7.
...
Пакет `MATH-4B2` завершает transient-фундамент и переводит контракт результата
на `asa-electronics-solver-v5`:
