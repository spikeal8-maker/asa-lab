...
- `DEC-INFO-007` — diagnostic badge является экранно-ориентированным слоем, не
  ...
  единого минимального читаемого размера.
...
- диаметр на экране определяется одной функцией для всего каталога:
  ...
  перестаёт уменьшаться ради читаемости. Центр привязан к правой верхней области
  повёрнутого bounding box компонента;
...
- Массивы с семантическим порядком (например, vertices провода) атомарны. Если
  ...
  revision, «другое устройство» или техническое описание конфликта. Она не
  дольше 4,5 секунды показывает только «Изменения сохранены в браузере», после
  чего освобождает экран. Повторное изменение снова запускает autosave и при
  необходимости повторяет это короткое сообщение.
...
### 17.2. Матрица покрытия каталога
