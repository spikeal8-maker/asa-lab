...
`rear-bar-highlight` на общем левом/правом выходном валу и светлую поверхность
внутри `bottom-shaft`. Верхний жёлтый выступ не является вращающимся валом и
остаётся неподвижным. Контрастная полоса проходит по всей доступной толщине
левого, правого и нижнего вала и выводится из signed RPM и принятого
`simulationTimeMs`: быстрый внутренний вал и медленный выходной вал имеют разные
спокойные presentation-диапазоны, а корпуса и оси не шатаются и не вращаются
целиком как пропеллер. Локальная diagnostic badge привязана к правому верхнему
краю жёлтого корпуса; длинный поперечный вал не участвует в расчёте её позиции.
`prefers-reduced-motion` останавливает движение полосы, не скрывая её и точное
число оборотов.
...
- Stage показывает signed `outputRpm` только из принятого transient-result;
