...
Оставшаяся часть `MATH-4B` включает адаптивное отклонение шага, переключаемые
топологии без перезапуска времени, NPN astable fixture, накопление тепла,
переход `warning → overheat → failure` и повторный расчёт после отказа. До её
приёмки статический `destructive_preview` не выдаётся за постоянное повреждение.
