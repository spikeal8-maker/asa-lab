...
- Массивы с семантическим порядком (например, vertices провода) атомарны. Если
  ...
  перезаписывается, локальный черновик сохраняется и UI показывает точный
  конфликт одного элемента.
...
- Это синхронизация документов, а не CRDT presence: одновременные курсоры и
  бесконфликтное редактирование одного и того же числового поля относятся к
  отдельному real-time этапу. До него одинаковое свойство никогда не
  разрешается молчаливым выбором последней записи.
