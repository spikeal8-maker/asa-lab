...
`bottom-shaft`. Их небольшое смещение выводится из signed RPM и принятого
`simulationTimeMs`: быстрый внутренний вал и медленный выходной вал имеют
разные спокойные presentation-диапазоны, а корпуса и оси не шатаются и не
вращаются целиком как пропеллер. `prefers-reduced-motion` останавливает это
движение, не скрывая числа.
