...
- `DEC-MATH-019` — RPM, направление и угловая фаза SVG вычисляются только из
  принятого transient state; независимая CSS-анимация запрещена.
...
шестерня служит только отображением того же observation.

Существующая SVG-группа `gear` вращается по фазе `omega_m`, сохраняя направление
и плавный разгон/выбег. Ни напряжение, ни `speedPercent`, ни CSS-duration не
