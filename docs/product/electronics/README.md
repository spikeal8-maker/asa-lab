...
- диаметр на экране определяется одной функцией для всего каталога:
  ...
  область фактически нарисованной альфа-маски после rotation/mirror, а не на
  прозрачный прямоугольник SVG; значок не имеет права висеть отдельно от детали
  или перекрывать соседний компонент;
...
`rear-bar-highlight`, `top-shaft-inner` и светлую поверхность внутри
`bottom-shaft`. Контрастная полоса движется поперёк уже нарисованной поверхности
вала и выводится из signed RPM и принятого `simulationTimeMs`: быстрый внутренний
вал и медленный выходной вал имеют разные спокойные presentation-диапазоны, а
корпуса и оси не шатаются и не вращаются целиком как пропеллер.
