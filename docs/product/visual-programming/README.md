...
## Установка, обновление и порт Scratch

Для deployment/repair сначала открыть [единый runbook](../../deployment/SCRATCH_INSTALLATION.md).
`blocks` — модуль ASA, `scratch` — сервис её основного Compose project. Термины
`standalone`, отдельный образ/origin и порт 4613 не разрешают вторую установку.
Preview/artifact-рецепты не являются путём обновления работающего сайта.
Сначала определить существующую установку и сохранить её project/root/порты;
при конфликте порта диагностировать владельца, не подбирать новый.
