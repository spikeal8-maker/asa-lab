...
### 7.1. Журнал версий проекта

Пользовательское название функции — **«Журнал версий»**. Это общий механизм
Project Core для всех предметных редакторов, а не отдельная реализация внутри
Electronics, Arduino, 3D или другого модуля.

Журнал открывается из меню карточки проекта. Владелец личного проекта и ученик
могут просматривать и восстанавливать собственные версии. Педагог получает тот
же viewer только для работ, доступных ему через класс, задание или проверку;
просмотр журнала сам по себе не даёт права изменять ученический проект.

Версия хранит только canonical структурированный payload модуля: объекты,
координаты, свойства, связи, пользовательский код, `moduleVersion` и
`schemaVersion`. SVG, растровые изображения компонентов, результаты симуляции,
звук и runtime-состояние в `ProjectVersion` не копируются. Viewer собирает
сцену из payload и общего каталога ресурсов. Для воспроизводимости старого вида
payload ссылается на стабильную identity/revision ресурса, а не содержит его
копию.

Контракт интерфейса:

- версии группируются по календарным дням и показывают номер, дату и время до
  минут;
- выбор строки открывает read-only preview точной версии, включая доступный в
  ней код;
- preview строится детерминированно из payload и не требует запуска полной
  предметной симуляции;
- действие «Восстановить» сначала фиксирует текущий draft как checkpoint
  «Перед восстановлением», затем создаёт новый draft из выбранной версии;
- восстановление никогда не изменяет и не удаляет прежнюю immutable version;
- отправленная работа и версия, на которую ссылается Submission, не подпадают
  под автоматическое удаление.

Autosave и журнал — разные потоки. Частый autosave обновляет mutable draft
батчами и не создаёт строку истории каждые 10–15 секунд. Автоматический
checkpoint создаётся по измеряемой политике (например, после периода активной
работы, при выходе, перед восстановлением и при сдаче) только если digest
изменился. Ручной checkpoint создаётся по явному действию пользователя.
Retention автоматически прореживает только промежуточные автоматические
checkpoints; ручные, отправленные и проверяемые версии сохраняются согласно
политике класса и организации.

Порядок реализации:

1. Расширить list/view API так, чтобы exact version открывалась в общем
   read-only Module Viewer без выдачи права на редактирование.
2. Добавить в «Журнал версий» группировку по дням, выбор строки, preview схемы
   или сцены и просмотр кода.
3. Ввести digest-deduplication и серверную политику автоматических checkpoint,
   отдельную от autosave.
4. Добавить role/RLS-тесты владельца, ученика и педагога, а также проверку
   non-destructive restore и открытия старой schema version.
5. Измерить рост хранилища и только после этого утвердить retention; хранение
   отдельной копии SVG или полного preview для каждой версии запрещено без
