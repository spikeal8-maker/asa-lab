...
- [`interface-catalog.html`](interface-catalog.html) — owner-каталог страниц, Electronics tools и parity capabilities;
- [`page-wireframes.html`](page-wireframes.html) — автоматически построенные визуальные схемы каждой catalogued page по Public/Portal/Classroom/Learner/Editor/Admin/Publication shell;
...
- [`../delivery/R0_OWNER_DECISION.md`](../delivery/R0_OWNER_DECISION.md) — шесть решений владельца;
...
Обзор интерфейсов, инструментов и parity:

```text
http://127.0.0.1:8080/docs/product/interface-catalog.html
```

Wireframe-схемы каждой catalogued page:

```text
http://127.0.0.1:8080/docs/product/page-wireframes.html
```

Первая страница показывает роли, routes, implementation states, actions, screenshots, Electronics tools и weighted parity scope. Вторая строит визуальную структуру каждой страницы из её shell-шаблона и честно показывает runtime status.

## Текущий owner action

1. Открыть оба визуальных каталога и проверить полноту экранов/инструментов/структуры страниц.
...
3. Открыть [`R0_OWNER_DECISION.md`](../delivery/R0_OWNER_DECISION.md) и принять/отклонить шесть решений; состояние фиксируется в [`R0_OWNER_DECISION.yaml`](../delivery/R0_OWNER_DECISION.yaml).
