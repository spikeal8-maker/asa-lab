...
async function classroomWithSeat() {
  ...
  const seat = (
    await admin.query(
      "INSERT INTO classroom_student_seats(tenant_id,classroom_id,display_label,login_handle,normalized_login_handle,safe_mode,status,created_by) VALUES($1,$2,'D3b learner',$3,$3,true,'active',$4) RETURNING id",
      [author.tenantId, classroom, `d3b-seat-${sequence}`, author.teacherId],
    )
  ).rows[0].id as string;
...
describe("E1-FIX-11D2 canonical Activity blocks", () => {
  ...
  it("accepts exact Electronics and 3D versions and rejects nonexistent, unsupported and foreign versions", async () => {
    const electronics = await publishActivity(
      author,
      principalId,
      "electronics",
    );
    ...
    const foreign = await publishActivity(
      outsider,
      outsiderPrincipalId,
      "electronics",
    );
  ...
  it("validates hidden Activity blocks, stores the draft and freezes only the visible exact pin", async () => {
    ...
    const threeD = await publishActivity(author, principalId, "three-d");
    const unsupported = await publishActivity(
      author,
      principalId,
      "manual-lab",
    );
    ...
    const draft = await admin.query(
      "SELECT blocks,content FROM course_lessons WHERE id=$1",
      [lessonId],
    );
    ...
    expect(
      (
        await admin.query(
          "SELECT course_lesson_blocks_plain_text($1::jsonb) AS content",
    ...
    const revision = Number(
      (
        await admin.query("SELECT draft_revision FROM courses WHERE id=$1", [
          courseId,
        ])
    ...
    const published = await admin.query(
      ...
      [principalId, courseId, revision, `d2:course:publish:${++sequence}`],
    );
  ...
  it("preserves the legacy lesson-level exact LearningActivityVersion pin", async () => {
    ...
    expect(
      (
        await admin.query(
          "SELECT learning_activity_version_id FROM course_lessons WHERE id=$1",
          [lessonId],
        )
    ...
    const published = await admin.query(
      "SELECT * FROM course_publish_v3($1,$2,$3,$4)",
      [principalId, courseId, revision, `d2:legacy:publish:${++sequence}`],
    );
    expect(published.rows[0].result_code).toBe("ok");
    const frozen = await admin.query(
...
describe("E1-FIX-11D3b Course Activity block materialization", () => {
  it("materializes two frozen Activity blocks into distinct runs, handouts and inherited participations", async () => {
    ...
    const { courseId, sectionId } = await newCourse(
      "D3b block materialization",
    );
    const blocks = [
      {
        id: "activity-block-a",
    ...
    expect(materialized.map((row) => row.learning_activity_version_id)).toEqual(
      [blockA.versionId, blockB.versionId],
    );
    expect(
      materialized.every((row) => row.source_course_lesson_id === runLesson.id),
    ).toBe(true);
    expect(new Set(materialized.map((row) => row.id)).size).toBe(2);
    expect(
      new Set(materialized.map((row) => row.source_classroom_assignment_id))
        .size,
    ).toBe(2);
    expect(
      materialized.every(
  ...
  it("preserves legacy lesson-level ActivityRun with a null block identity", async () => {
    ...
    expect(runs[0].source_classroom_assignment_id).toBe(
      runs[0].classroom_assignment_id,
    );
  });
});
