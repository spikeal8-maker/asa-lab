...
async function createLearner(classroomId: string, label: string) {
  const seat = await admin.query(
    ...
    [owner.tenantId, classroomId, label, `a6-${++sequence}`, owner.teacherId],
  );
...
describe('LRN-A6-01 class session and attendance foundation', () => {
  it('creates one scheduled session, records canonical attendance and preserves completed history', async () => {
  ...
  it('does not allow an unrelated educator to mutate another classroom session', async () => {
    const classroomId = await createClass();
    const session = (
      await app.query(
        `SELECT * FROM class_session_create(
