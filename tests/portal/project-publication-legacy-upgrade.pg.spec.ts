...
it('converges populated legacy publications without losing reactions or Collections', async () => {
  ...
  try {
    ...
    const baseline = planned.filter((entry: { version: string }) => entry.version < '0144');
    const convergence = planned.filter((entry: { version: string }) => entry.version === '0144');
    expect(baseline.at(-1)?.version).toBe('0143');
