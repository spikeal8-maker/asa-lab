...
if (!output) throw new Error('ASA_3D_BENCHMARK_REPORT must name the generated receipt');
...
if (!cases.some((entry) => entry.tier === 'stress')) {
  throw new Error('benchmark receipt must include the stress tier');
}
if (!cases.every((entry) => entry.deterministic)) {
  throw new Error('legacy evaluator produced a non-deterministic receipt');
}
