...
REQUIRED_CLAUSES = {
    ...
    ACCESS: ["class.credentials.read_current", "class.credentials.rotate", "class.sessions.revoke", "AES-256-GCM", "HMAC-SHA-256", "Retry-After", "shared rate-limit state", "Регистр значим:", "[A-Za-z0-9]{4,10}", "effective exact-class staff scope", "Временно подменяющий педагог", "2346789ACDEFGHJKMNPQRTUVWXYacdefghjkmnpqrtuvwxy", "credential_storage_unavailable", "credential_version_conflict", "v1|tenant_id|class_id|seat_id|credential_version", "lookup_key_id", "CLASSROOM_CODE_SECRET", "ASA_STUDENT_CODE_PROTECTION_MODE=compat", "legacy_predictable_active=0", "Один Account — одна личная оболочка", "публичные read-only «Сообщество» и «Знания»", "UX после успешного linking", "MAX/другие внешние providers привязываются к Account", "Organization Workspace — отдельный рабочий контекст", "UI-37"],
