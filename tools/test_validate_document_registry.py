...
class DocumentRegistryTests(unittest.TestCase):
    def test_historical_snapshot_is_byte_pinned(self):
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            data = registry(root)
            snapshot = root / "old.md"
            snapshot.write_bytes(b"previous canonical\r\n")
            data["documents"].append({
                "id": "OLD", "path": "old.md", "scope": "global", "kind": "target",
                "status": "superseded", "authority": "none", "read_when": ["history"],
                "superseded_by": "POLICY",
                "snapshot_sha256": hashlib.sha256(snapshot.read_bytes()).hexdigest(),
            })
            self.assertEqual(MODULE.validate_registry(root, data), [])
            snapshot.write_bytes(b"previous canonical\n")
            self.assertTrue(any("checksum mismatch" in e for e in MODULE.validate_registry(root, data)))
