CREATE OR REPLACE FUNCTION learning_grade_label_for_run(p_run uuid,p_percentage integer)
RETURNS varchar LANGUAGE sql STABLE SECURITY DEFINER SET search_path=pg_catalog,pg_temp AS $$
 SELECT band->>'label' FROM public.activity_runs run JOIN public.grading_scheme_versions scheme ON scheme.id=run.grading_scheme_version_id
 CROSS JOIN LATERAL jsonb_array_elements(scheme.bands) band WHERE run.id=p_run AND (band->>'minBasisPoints')::integer<=p_percentage
 ORDER BY (band->>'minBasisPoints')::integer DESC LIMIT 1;
$$;
REVOKE ALL ON FUNCTION learning_grade_label_for_run(uuid,integer) FROM PUBLIC,asalab_app;
